...
# The type and length of each column determines the data type used to write it out
...
writeOut <- function(connection, x) {
  ...
  varTypeVector = purrr::map(x, ~max(stringr::str_length(.x))) %>%
    ...
    left_join(purrr::map(x, class) %>% 
                as.data.frame() %>%
                pivot_longer(cols = everything(),
                             names_to = "column", values_to = "class"),
              by = "column") %>%
    mutate(columnType = case_when(max_length < 255 & class == "character" ~ "VARCHAR(255)",
                                  max_length < 255 & class == "integer" ~ "INT",
                                  max_length < 255 & class == "numeric" ~ "DOUBLE",
...
database <- "data/prairie-fen-database.accdb"
connection <- odbcConnectAccess2007(database)
  # Rebuild the connection each time you open R
...
# but because we don't have a primary key Access actually won't let us update either
...
write.table(prairie_sedge_points, "output/prairie_sedge_points.txt",
